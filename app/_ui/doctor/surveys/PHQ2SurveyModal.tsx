...
interface IPHQ2SurveyModal {
  patientHistoryId?: number;
  openSurveyModal: boolean;
  setOpenSurveyModal: Function;
}

export default function PHQ2SurveyModal(props: IPHQ2SurveyModal) {
  ...
  async function getSurveyData() {
    try {
      setLoading(true);
      if (patientHistoryId) {
        const result = await getSurveyByName(SurveyName.PHQ2, patientHistoryId);
        setSurveyData(result);
        setSurveyResponses(result.responses);
      } else {
        const result = await getSurveyByName(SurveyName.PHQ2);
        setSurveyData(result);
      }
      setLoading(false);
    } catch (error) {
