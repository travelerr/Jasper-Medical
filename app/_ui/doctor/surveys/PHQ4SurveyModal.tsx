...
export default function PHQ4SurveyModal(props: IPHQ4SurveyModal) {
  ...
  async function getSurveyData() {
    try {
      setLoading(true);
      if (patientHistoryId) {
        const result = await getSurveyByName(SurveyName.PHQ4, patientHistoryId);
        setSurveyData(result);
        setSurveyResponses(result.responses);
      } else {
        const result = await getSurveyByName(SurveyName.PHQ4);
        setSurveyData(result);
      }
      setLoading(false);
    } catch (error) {
