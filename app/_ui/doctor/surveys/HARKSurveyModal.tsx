...
interface IHARKSurveyModal {
  patientHistoryId?: number;
  openSurveyModal: boolean;
  setOpenSurveyModal: Function;
}

export default function HARKSurveyModal(props: IHARKSurveyModal) {
  ...
  const onSubmit: SubmitHandler<any> = async (data) => {
    ...
    surveyData.questions.forEach((q) => {
      ...
      if (q.type === SurveyQuestionType.Boolean) {
        response.responseBool = data[q.id] === "true";
      ...
      if (q.type === SurveyQuestionType.Text) {
        response.responseText = data[q.id];
      }
  ...
  async function getSurveyData() {
    try {
      setLoading(true);
      if (patientHistoryId) {
        const result = await getSurveyByName(SurveyName.HARK, patientHistoryId);
        setSurveyData(result);
        setSurveyResponses(result.responses);
      } else {
        const result = await getSurveyByName(SurveyName.HARK);
        setSurveyData(result);
      }
      setLoading(false);
    } catch (error) {
  ...
  return (
    ...
    >
      <FlowBiteModal.Header>
        <div>
          <div>{SurveyName.HARK}</div>
          <div>
            <div>
              <span>
                National Domestic Violence Hotline:{" "}
                <span className="text-sky-500">1.800.799.SAFE (7233)</span>
              </span>
            </div>
            <div>
              <a
              ...
              {surveyData.questions
                ...
                .map((q, index) => (
                  <div className="w-1/2 flex my-5" key={q.id}>
                    <div className="text-3xl mr-3">{index + 1}</div>
